...
/** 
 * Función para pasar por url un array
 */
...
/**
 * Escapea caracteres para el ilike
 */
function addEscapeosParaLike($cosa) {
	$cosa = str_replace('\\','\\\\',$cosa);
	$cosa = str_replace('%','\%',$cosa);
	$cosa = str_replace('_','\_',$cosa);
	$cosa = str_replace('\'','\\\'',$cosa);
	return addslashes(trim($cosa));
}
?>