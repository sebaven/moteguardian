		...
		{	            
                ...
                if ($numrows ==0){
                        ...
                        foreach($columnas as $valor){
                                $a1="";$a2="";$img="";
                                if(isset($this->estilo_titulo) && $this->estilo_titulo)
                                        $thead .="<th class='$this->estilo_titulo'>";
                                else
                                        $thead .="<th>";
                        ...
                        while ( ($row = $this->db->fetch_array($rs)) && ($count<$cfilas) ) {
                                ...
                                foreach($columnas as $valor){
                                        ...
                                        if($valor["dato_align"]){
                                                $dato_align="align='".$valor["dato_align"]."'";
                                        }
                                        if($valor["dato_width"]){
                                                $dato_width="width='".$valor["dato_width"]."'";
                                        }
                                        if ($valor["datos"]){
                                        ...
                                        if($valor["dato_foto"]){
                                        	if(isset($row[$valor['dato_foto']])) {
                                        		if($row[$valor['dato_foto']]==TRUE_){
                                        			$img ="<img src='../../imagenes/listado/exitoTarea/1.png"."' border='0' align='absmiddle'>";
                                        		} else if($row[$valor['dato_foto']]==FALSE_){
                                        			$img ="<img src='../../imagenes/listado/exitoTarea/0.png' border='0' align='absmiddle'>";	
                                        		}	
                                        	} else {                                        		
        ...
        function imprimir_listado(){
			...
			";			
                ...
                if($this->numregistros()){
                        ...
                        $s = "<table cellspacing=1 class='".$this->estilo."'>".$this->alistado["thead"].$this->alistado["tfoot"].$this->alistado["tbody"]."</table>";
...
/*                if ($this->db->motor=='mssql'){
        ...
        function navegador($datos,$offset,$numrows,$cfilas,$maxpag){        	
                ...
                //arrastro las variables que pasan por get
                $ex_pasaget= array("offset","borrar","publicar","despublicar");
                foreach( $datos as $param => $valor ) {
                ...
                //			pero solo querramos mostrar una p�gina
                if (($total_pages>1) && ($maxpag >1)){
                        ...
                        if ($offset>0 && ($offset <= $numrows)) {                                         // previene que no aparezca el link de pagina anterior
                                        $prevoffset=$offset-$cfilas;
                                                $str_nav = $str_nav."<a  href=\"$PHP_SELF?offset=0$pasa_get\"><img src='".$this->path_image."boton_inicio.gif' alt='Inicio' style='border:0px; text-align:center;'></a> &nbsp; \n";
                                        if ($intermaxpag>=1){
                                                $str_nav = $str_nav."<a  href=\"$PHP_SELF?offset=".((($maxpag*$cfilas)*($intermaxpag))-($maxpag*$cfilas))."$pasa_get\"><img src='".$this->path_image."boton_retro.gif' alt='Retroceder $maxpag p&aacute;ginas' style='border:0px; text-align:center;'></a> &nbsp;\n";
                                        }
                                        $str_nav = $str_nav."<a  href=\"$PHP_SELF?offset=$prevoffset$pasa_get\"><img src='".$this->path_image."boton_atras.gif' alt='P&aacute;gina anterior' style='border:0px; text-align:center;'></a> &nbsp; \n";
                        }
                        ...
                        for ($i=$init;$i<=($pages);$i++) {
                                ...
                                $str_nav = $str_nav."<a class='paginado' href=\"$PHP_SELF?offset=$newoffset$pasa_get\">".((($offset/$cfilas)+1==$i) ? "<strong class='actual'>$i</strong>" : "$i")."</a> &nbsp; \n";
                        }
                        ...
                        if (!(($offset/$cfilas)==$pages) ) {
                                ...
                                if((($numrows - $offset) > $cfilas)  && ($offset < $numrows)){
                                        $str_nav = $str_nav."<a  href=\"$PHP_SELF?offset=$newoffset$pasa_get\"><img src='".$this->path_image."boton_adelante.gif' alt='P&aacute;gina siguiente' style='border:0px; text-align:center;'></a> &nbsp;\n";
                                        if ((((($intermaxpag+1)*$maxpag)*$cfilas)==($pages*$cfilas))&&($total_pages<>$pages)){
                                                $str_nav = $str_nav."<a  href=\"$PHP_SELF?offset=".((($intermaxpag+1)*$maxpag)*$cfilas)."$pasa_get\"><img src='".$this->path_image."boton_avance.gif' alt='Adelantar $maxpag P&aacute;ginas' style='border:0px; text-align:center;'></a> &nbsp;\n";
                                        }
                                        $str_nav = $str_nav."<a  href=\"$PHP_SELF?offset=".(($total_pages*$cfilas)-$cfilas)."$pasa_get\"><img src='".$this->path_image."boton_fin.gif' alt='Final' style='border:0px; text-align:center;'></a>\n";
                                }
                        }                      
                }       
